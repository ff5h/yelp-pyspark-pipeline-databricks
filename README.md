...
```mermaid
graph TD
    A([Початок: Вхідне слово]) --> B{Є в списку стоп-слів?}
    B -- Так --> C[Пропустити: Просте слово]
    B -- Ні --> D[Пошук у частотному словнику]

    D --> E{Частотність > Порогу?}
    E -- Так --> F[Позначити як: Просте слово]
    E -- Ні --> G[Позначити як: Потенційно складне]

    G --> H[Звернення до словника синонімів]
    H --> I{Чи є простіші синоніми?}
    
    I -- Так --> J[Запропонувати заміну синонімом]
    I -- Ні --> K[Позначити як: Термін / Рідкісне слово]
